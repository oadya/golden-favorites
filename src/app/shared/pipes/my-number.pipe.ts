...
export class MyNumberPipe implements PipeTransform {
  ...
  private DECIMAL_SEPARATOR_PRINT: string;
  private THOUSANDS_SEPARATOR: string;

  constructor() {
    // TODO comes from configuration settings
    this.DECIMAL_SEPARATOR = '.';
    this.DECIMAL_SEPARATOR_PRINT = ',';
  ...
  public transform(value: number | string, fractionSize: number = 2): string {
    ...
    if (value !== '') {
      //console.log(' ** myNumberFormat **')
      let [integer, fraction = ''] = (value || '').toString()
        .split(this.DECIMAL_SEPARATOR);

      fraction = fractionSize > 0
        ? this.DECIMAL_SEPARATOR_PRINT + (fraction + PADDING).substring(0, fractionSize)
        : '';
